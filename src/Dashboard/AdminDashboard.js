            ...
            <nav className="bg-lightWhite py-4 mb-12">
              <div className="mx-32">
                <div className="flex flex-row justify-between">
                  ...
                  <div style={{fontSize: '.875rem'}} className="relative group mw-64" onClick={() => this.openDropdown('profileDropdown')}>
                    <div className="flex items-center cursor-pointer text-blue border border-white border-b-0 hover:border-grey-light rounded-t-lg py-3 px-2">
                       admin@um6p.ma 
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" className="fill-current h-4 w-4">
                        <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"> </path>
                      </svg>
                    </div>
                    <div id='profileDropdown' style={{width: 200}} className="items-center absolute border border-t-0 rounded-b-lg p-1 bg-white p-2 w-full invisible">
                      ...
                      <button onClick={SignOut} className="w-full text-left px-4 py-2 block text-black hover:bg-grey-lighter focus:outline-none">Se déconnecter</button>
                    </div>
                  </div>
                </div>
              </div>
...
//   </div>
// </div>
<div class="flex-auto mx-2 bg-CleanGray p-4 rounded">
  <div>
    <div>
    <div class="flex flex-row mb-2">
      <img src={props.img} alt="" class="w-12 h-12" style={{width: '20%'}} />
      <h3 style={{width: '80% !important'}} class="flex-auto flex items-center text-mainBlue px-2 text-xl rounded bg-white">{props.name}</h3>
    </div>
    </div>
    </div>
  </div>
...
const Station = (props) => {
  return(
    <div>
      {props.plates.map((plate, key) => <Plate name={plate.name} img={plate.imgUrl} key={key} /> )}
    </div>
  )
}
const Meal = (props) => {
  ...
  return (
    ...
    <div style={{width: '200px !important'}} className="w-2/12">
      <h2 style={{padding: '200px !important'}} className="MealHeading flex items-center justify-center bg-mainBlue text-white text-2xl rounded font-bold h-full">{props.heading}</h2>
    </div>
    {
